...
net_mainwindow::net_mainwindow(QWidget* parent)
    : QDialog(parent), ui(new Ui::net_mainwindow) {
  ui->setupUi(this);
...
net_mainwindow::~net_mainwindow() { delete ui; }

void net_mainwindow::setUI() {
  // QString img1 =
  // "C:/Users/SITP/Desktop/sun01/YingGuang_ISN_10008/3rd_app/00-internal1-sun/images/LCVNIR-20250613_145257-00001.png";
  // QPixmap pm(img1);
...
void net_mainwindow::setConnection() {
  ...
  connect(ui->btn_selectFile, &QPushButton::clicked, this, [this]() {
    ...
    // 读取配置
    if (ConfigManager::instance()
            .getValue("matlabSetting/ReadFile_Path")
            .isValid())
      default_path1 = ConfigManager::instance()
                          .getValue("matlabSetting/ReadFile_Path")
                          .toString();  // 脚本注入目录
  ...
  connect(ui->btn_Start, &QPushButton::clicked, this,
          &net_mainwindow::onStartProcess);

  // 停止按钮
  connect(ui->btn_Stop, &QPushButton::clicked, this,
          &net_mainwindow::onStopProcess);
...
void net_mainwindow::onStartProcess() {
  // 整理包，并检查文件
  datapack_matlab1.input_dat_full =
      ui->text_file->text();  // path/LCVNIR-20250613_144349-00001.dat
  QFileInfo info1(datapack_matlab1.input_dat_full);
  datapack_matlab1.base_name =
      info1.baseName();  // LCVNIR-20250613_144349-00001
  datapack_matlab1.integ_time = ui->combo_integ_time->currentText();  // 18ms
  QString input_dir =
      info1.path();  // 未完成路径确认，记得确认matlab输出路径是否正确
  datapack_matlab1.out_folder = QDir(input_dir).filePath(
      datapack_matlab1.base_name);  // path/LCVNIR-20250613_144349-00001/
  datapack_matlab1.out_png_name_full =
      QDir(datapack_matlab1.out_folder)
          .filePath(datapack_matlab1.base_name +
                    ".png");  // outpath/LCVNIR-20250613_145257-00001.png
  datapack_matlab1.out_L1B_dat_name_full =
      QDir(datapack_matlab1.out_folder)
          .filePath(
              datapack_matlab1.base_name +
              "_L1B.dat");  // outpath/LCVNIR-20250613_145257-00001_L1B.dat

  // 生成先导matlab脚本
  QString scriptfull =
      QDir(ui->text_ScriptRootPath->text()).filePath("test_1.m");
  writeMatlabScript(scriptfull, datapack_matlab1.input_dat_full,
                    datapack_matlab1.integ_time);
  ...
  QStringList matlabArgs = {
      ...
      // R"(run('C:\Users\SITP\Desktop\sun01\荧光临时\test0.m');exit;)"   //
      // 未完成，最后得改
...
void net_mainwindow::onMatlabFinished(int, QProcess::ExitStatus) {
  ...
  // 文件安全检查
  QFileInfo fi1(datapack_matlab1.out_L1B_dat_name_full);
  if (!fi1.exists()) {
  ...
  datapack_pos_zx1.out_csv_full =
      QDir(info2.absolutePath()).filePath(info2.baseName()) + "_POS.csv";

  // 准备启动POS的EXE
  const QString appDir = QCoreApplication::applicationDirPath();
  const QString exePath =
      QDir(appDir).filePath("apps/20pos-zx/POS_YG_Core.exe");
  m_p2_pos_zx->setProgram(exePath);
  m_p2_pos_zx->setArguments(
      {datapack_pos_zx1.in_dat_full,
       datapack_pos_zx1.out_csv_full});  // 如需参数，填在这里
  m_p2_pos_zx->setProcessChannelMode(QProcess::MergedChannels);
  m_p2_pos_zx->start();

  qDebug() << datapack_pos_zx1.in_dat_full << " "
           << datapack_pos_zx1.out_csv_full;
...
void net_mainwindow::onPos_zs_Finished(int, QProcess::ExitStatus) {
  ...
  // datapack_jihe1.out_folder =
  //     QDir(info_in.path()).filePath("out_jihe").replace("\\", "/");

  datapack_jihe1.out_folder = QDir(info_in.path()).filePath("out_jihe");

  datapack_jihe1.out_L_jpg_full =
      QDir(datapack_jihe1.out_folder)
          .filePath(datapack_jihe1.basename1 + "_L_RGB.jpg");

  qDebug() << datapack_jihe1.in_csv_full << " " << datapack_jihe1.in_dat_full
           << " " << datapack_jihe1.out_folder << " "
           << datapack_jihe1.out_L_jpg_full;
  ...
  // 启动程序
  const QString appDir = QCoreApplication::applicationDirPath();
  // const QString exePath = QDir(appDir).filePath("apps/30jihe/bes.exe"); //
  // 未完成，最后得改
  const QString exePath =
      QDir(appDir).filePath("apps/30jihe/LCVNIR_rotate.exe");

  QStringList exeArgs = {"--pos", datapack_jihe1.in_csv_full,
  ...
  const QString Proj_LIB_Path = QDir(appDir).filePath("../../database/proj");
  // env.insert("PROJ_LIB", R"(C:\Users\SITP\Desktop\sun01\YingGuang_ISN_10008\out\database\proj)");   // 或者从你的配置里读
  env.insert("PROJ_LIB", Proj_LIB_Path);   // 或者从你的配置里读
...
void net_mainwindow::onJihe_Finished(int, QProcess::ExitStatus) {
  ...
  // QString img1 =
  // "E:/Y.荧光/matlab处理/示例数据/48ms/LCVNIR-20250613_145257-00001/LCVNIR-20250613_145257-00001_L1B_L_RGB.jpg";
  // // 未完成，要改
...
void net_mainwindow::writeMatlabScript(const QString& scriptPath,
  ...
  const QString header =
      QStringLiteral("% 文件编码：GB18030（请勿另存为UTF-8）\r\n");
